...
def get_gene_coordinates(id, assembly):
    species = _GENOME_TO_SPECIES[assembly]
    url = '{ensembl}lookup/symbol/homo_sapiens/{id}?content-type=application/json'.format(
        ensembl=_ENSEMBL_URL,
        id=id
    )
    try:
...
def region_search(context, request):
    ...
    if annotation != '*':
        ...
        if region.startswith('rs'):
            ...
            chromosome, start, end = get_gene_coordinates(region, assembly)
    else:
        chromosome, start, end = ('', '', '')
    ...
    if len(file_uuids):
        query = get_filtered_query('', [], set(), ['Annotation'])
        del query['query']
        ...
        query['aggs'] = set_facets(_FACETS, used_filters,['Annotation'])
        schemas = (types[item_type].schema for item_type in ['Annotation'])
        es_results = es.search(
            body=query, index='snovault', doc_type='annotation', size=size
        )
        log.warn(es_results['hits']['hits'])
        result['@graph'] = list(format_results(request, es_results['hits']['hits']))
        result['total'] = total = es_results['hits']['total']
        result['notification'] = 'Result'
        result['facets'] = format_facets(peak_results, _FACETS, used_filters, schemas, total)
        ...
        for bucket in aggregations['assembly']['assembly']['buckets']:
            if bucket['doc_count'] > 0:
                ...
                or assembly in request.params.getall('assembly'):
                    ...
                    hub_url = request.route_url('batch_hub',search_params=search_params,txt='')
                    browser_urls = {}
                    browser_urls1 = {}
                    pos = None
...
def set_facets(facets, used_filters, doc_types):
    """                                                                                                                                                                             
    ...
    return aggs
def get_filtered_query(term, search_fields, result_fields, doc_types):
    return {
        'query': {
            'query_string': {
            'query': term,
